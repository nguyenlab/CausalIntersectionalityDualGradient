...
from src.vilio.modeling_bertU_gradient import BertU
...
class ModelU(nn.Module):
    def __init__(
        self,
        args=args,
        max_seq_len=100 + args.num_features,
        ...
        if tr_name.startswith("roberta"):
            self.model, loading_info = RobertaU.from_pretrained(
                self.tr_name, img_dim=2048, output_loading_info=True
            )
        elif tr_name.startswith("bert"):
            self.model, loading_info = BertU.from_pretrained(
                self.tr_name, img_dim=2048, output_loading_info=True
            )
        ...
        if self.model.config.type_vocab_size == 1:
            ...
            self.model.embeddings.token_type_embeddings = nn.Embedding(
                2, self.model.config.hidden_size
            )
            # Initialize it
    ...
    def preprocess_roberta(self, sents, visual_feats, num_features, tokenizer):
        ...
        input_ids = pad_sequence(
            iids, batch_first=True, padding_value=1
        )  # [1] is Roberta's padding token
    ...
    def preprocess_bert(self, sents, visual_feats, num_features, tokenizer):
        ...
        return input_ids, img_feats, img_pos_feats, attn_masks, gather_index, tokens_all
    ...
    def forward(
        ...
        if self.tr_name.startswith("roberta"):
            (
                ...
                sents, visual_feats, self.num_features, self.tokenizer
            )
            ...
            (
                ...
                gather_index,
                tokens_all,
            ) = self.preprocess_bert(
                sents, visual_feats, self.num_features, self.tokenizer
            )

        seq_out, pooled_output, intermediates = self.model(
            input_ids.cuda(),
            None,
            img_feats.cuda(),
            img_pos_feats.cuda(),
        ...
        output = (output, intermediates, tokens_all)
    ...
    def save(self, path):
        torch.save(self.model.state_dict(), os.path.join("%s_U.pth" % path))
