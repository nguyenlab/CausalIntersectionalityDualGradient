...
# from transformers.modeling_bert import BertEncoder as OrgBertEncoder
...
# added for scoring
from attattr.attattr import scaled_input, attention_attribute
from modality_evaluation.highlight_sample import highlight_sample
from modality_evaluation.metric import df_evaluator
from modality_evaluation.modality_score import (
    compression,
    modality_generator,
    modality_scorer,
    modality_summarizer,
...
from modality_evaluation.preprocessing import OrderedHMDataset, generate_confounders
...
def get_tuple(
    ...
    drop_last=False,
    batched=True,
    label_path="path/to/confouders.parquet",
    modality="img",
    generate_negatives: bool = True,
) -> DataTuple:

    if generate_negatives:
        _ = generate_confounders(label_path, modality=modality, split=splits)
    else:
        pass

    dset = HMDataset(splits)

    tset = HMTorchDataset(splits, gen_neg=generate_negatives)
    # commented out for lmdb issue
    ## https://github.com/pytorch/examples/issues/526
    # torch.save(tset, "/content/drive/MyDrive/vilio/test.pth")
    if batched:
        print(f"Size before batching: {len(tset)}")
        tset = OrderedHMDataset(
            tset,
            label_path,
            modality=modality,
            split=splits,
        ...
        # torch.save(tset, "/content/drive/MyDrive/vilio/test2.pth")
        print(f"Size after batching: {len(tset)}")
        ...
        out = DataTuple(dataset=dset, loader=data_loader, evaluator=evaluator)
    else:
        evaluator = HMEvaluator(tset)
        data_loader = DataLoader(
            tset,
...
def replace_text_encoder(model_name, device):
    # # if Path("encoder.bin").is_file():
    # #     print("Loading bert.bin")
    # #     model = OrgBertModel()
    # #     model = model.load_state_dict("encoder.bin")
    # # else:
    # #     print("Loading encoder from pretrained model")
    # #     model = OrgBertModel.from_pretrained(model_name).encoder
    # #     # model = OrgBertEncoder.from_pretrained(model_name).encoder
    # #     torch.save(model.state_dict(), "bert.bin")
    # model = OrgBertModel.from_pretrained(model_name).encoder
    # model_dict = model.state_dict()
    # # model_dict = torch.load("bert.bin")
    # config = BertConfig.from_pretrained(model_name)
    # # model_attr = BertModel(config)
    # encoder = BertEncoder(config)
    # # encoder = model_attr.load_state_dict(model_dict)
    # encoder = encoder.load_state_dict(model_dict)
    # # encoder = model_attr.encoder
...
class HM:
    def __init__(self):
        ...
        if args.contrib:
            self.optim = SWA(
                self.optim, swa_start=self.t_total * 0.75, swa_freq=5, swa_lr=args.lr
            )
                    ...
                    if ups % 250 == 0:
                        ...
                        log_str = (
                            ...
                            % (
                                epoch,
                                ups,
                                evaluator.evaluate(id2ans) * 100,
                                evaluator.roc_auc(id2prob) * 100,
                                total_loss,
                            )
                        )
    ...
    def predict(self, eval_tuple: DataTuple, dump=None, out_csv=True):
        ...
        modality_scores = {"all": [], "text": [], "image": [], "cross": []}
        if args.summary not in ["avg", "sum"]:
            summary = None
        else:
        ...
        for i, datum_tuple in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                if args.model == "O":
                    logit, enc_out, tokens_all = self.model(
                        sent, (feats, boxes), is_logsoftmax=args.logsoftmax
                    )  # (last hidden,(attention[layer]))
                else:
                    logit, enc_out, tokens_all = self.model(
                        sent, (feats, boxes)
                    )  # (last hidden,(attention[layer]))
                text_lengthes = [len(tokens) for tokens in tokens_all]
            ...
            atts = enc_out[0][-1]  # attention from last layer

            self.model.train()
            atts.requires_grad_(True)
            if args.swa:
                self.swa_model.train()

            scaled_atts = scaled_input(atts.to("cpu"), self.num_steps)
            gradient = None
            for s in range(self.num_steps):
                scaled_att = scaled_atts[s].to(self.device)
                logit, enc_out, _ = self.model(
                    sent,
                    (feats, boxes),
                    attention_weights=scaled_att,
                )

                gradient = (
                    enc_out[1].to("cpu")
                    if gradient is None
                    else enc_out[1].to("cpu") + gradient
                )

            # Attribute Scoring
            atts = atts.to("cpu")
            attattr = attention_attribute(
                gradient, atts, self.num_steps, norm_size=args.conf_unit_size
            )

            # modality scoring
            head_num, input_length = attattr.shape[1], attattr.shape[2]
            modalities = modality_generator(
                head_num,
                input_length,
                text_lengthes,
            )
            if args.extract_type == "attention":
                score = atts
            elif args.extract_type == "gradient":
                score = gradient
            else:
                score = attattr
            score = zero_exception_loc(score, text_lengthes)
            modality_score = modality_scorer(score, modalities, "cpu", summary)
            modality_score_raw = modality_scorer(score, modalities, "cpu", None)
            if args.highlight_sample:
                _ = highlight_sample(
                    ...
                    max_highlight_text_num=args.max_highlight_text_num,
                    max_highlight_image_num=args.max_highlight_image_num,
                    extract_type=args.extract_type,
                    tr_name=args.tr,
                )
            else:
                pass
            modality_score["all"] = compression(score, summary)
            for mod in modality_score.keys():
                modality_scores[mod].append(modality_score[mod])

        modality_scores = modality_summarizer(modality_scores, summary)

        if dump is not None:
            if out_csv is True:  # out_csv == True
                dump_filename = dump.split(".")[0]
                df = evaluator.dump_csv(
                    id2ans, id2prob, modality_scores, dump, args.conf_modality
                )
                df_label = pd.read_parquet(args.label_path, engine="pyarrow")
                _ = df_evaluator(
                    df_label,
                    df,
                    conf_modality=args.conf_modality,
                    export_path=f"{dump_filename}.html",
                    preds_threshold=args.preds_threshold,
                    pick_correct_labels=args.pick_correct_labels,
                )
            else:
        ...
        return id2ans, id2prob, modality_scores

    def evaluate(self, eval_tuple: DataTuple, dump=None):
        """Evaluate all data in data_tuple."""
        id2ans, id2prob, modality_scores = self.predict(eval_tuple, dump=dump)

        acc = eval_tuple.evaluator.evaluate(id2ans)
        roc_auc = eval_tuple.evaluator.roc_auc(id2prob)

        return acc, roc_auc, modality_scores

    def save(self, name):
        if args.swa:
            torch.save(
                self.swa_model.state_dict(), os.path.join(self.output, "%s.pth" % name)
            )
        else:
            torch.save(
                self.model.state_dict(), os.path.join(self.output, "%s.pth" % name)
            )

    def load(self, path, text_only=False):
        ...
        if text_only:
            self.model.model.encoder = replace_text_encoder(args.tr, self.device)
...
def main():
    ...
    if args.loadfin is not None:
        hm.load(args.loadfin, args.text_only)
    ...
    if args.test is not None:
        ...
        for split in args.test.split(","):
            eval_tuple = get_tuple(
                split,
                modality=args.conf_modality,
                bs=args.batch_size,
                conf_num=args.conf_unit_size,
                shuffle=False,
                drop_last=False,
                label_path=args.label_path,
            )
            # Anthing that has no labels:
            ## todo: split-related if statements to one
            dump = os.path.join(
                args.output, "{}_{}_{}.csv".format(args.conf_modality, args.exp, split)
            )
            if "test" in split:
                hm.predict(eval_tuple, dump=dump)
