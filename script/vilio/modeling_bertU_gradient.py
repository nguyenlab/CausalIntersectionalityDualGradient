...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, attention_weights=None):
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, attention_weights=None):
        self_output, atts = self.self(
            input_tensor,
            attention_mask,
            attention_weights=attention_weights,
        )
        attention_output = self.output(self_output, input_tensor)
        return (attention_output, atts)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, attention_weights=None):
        attention_output, atts = self.attention(
            hidden_states,
            attention_mask,
            attention_weights=attention_weights,
        )
...
class UniterTextEmbeddings(nn.Module):
    ...
    def forward(
        self, input_ids, position_ids=None, token_type_ids=None, inputs_embeds=None
    ):
        ...
        position_embeddings = position_embeddings.repeat(
            token_type_embeddings.size(0), 1, 1
        )
...
class UniterEncoder(nn.Module):
    ...
    def forward(
        ...
        all_encoder_layers, all_atts = [], []
        hidden_states = input_
        for layer_module in self.layer:
            hidden_states, atts = layer_module(
                hidden_states,
                attention_mask,
                attention_weights=attention_weights,
            )
        ...
        if not output_all_encoded_layers:
            all_encoder_layers.append(hidden_states)
            all_atts.append(atts)
        return (all_encoder_layers, all_atts)
...
class BertU(BertPreTrainedModel):
    ...
    def forward(
        ...
        encoded_layers, atts = self.encoder(
            embedding_output,
        ...
        if not output_all_encoded_layers:
            ...
            pass
        # todo: consistency with output_all_encoded_layers
        ...
        if attention_weights is not None:
            ...
            outputs = (sequence_output, pooled_output, (atts, gradients))
        else:
            outputs = (sequence_output, pooled_output, (atts,))
