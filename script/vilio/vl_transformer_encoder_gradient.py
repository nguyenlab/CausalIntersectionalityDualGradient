...
def multi_head_attention(
    ...
    def __combine_heads(x):
        ...
        return layers.reshape(
            x=trans_x, shape=[0, 0, trans_x.shape[2] * trans_x.shape[3]], inplace=True
        )

    def scaled_dot_product_attention_gradient(
        q, k, v, attn_bias, d_key, dropout_rate, weights_org=None
    ):
        """
        ...
        if not weights_org:
            """
            in case this is first calculation, vilio code is executed
            """
            ...
            """
            in case weight is pre-calculated, nothing to do before w*v
            """
            weights = weights_org
        out = layers.matmul(weights, v)
        if weights_org:
            """
            in case weight is pre-calculated, gradient would be obtained
            """
            gradient = autograd.grad(out, weights)
        else:
            gradient = None
        return out, (weights, gradient)
    ...
    ctx_multiheads, w_g = scaled_dot_product_attention_gradient(
        q, k, v, attn_bias, d_key, dropout_rate
    )
    weights, gradients = w_g[0], w_g[1]
    print(f"weights: {weights.shape if weights else weights}")
    print(f"gradients: {gradients.shape if gradients else gradients}")
    paddle.disable_static()
    ctx_multiheads, w_g = scaled_dot_product_attention_gradient(
        q, k, v, attn_bias, d_key, dropout_rate, weights
    )
    weights, gradients = w_g[0], w_g[1]
    print(f"weights v2: {weights.shape if weights else weights}")
    print(f"gradients v2: {gradients.shape if gradients else gradients}")
...
def positionwise_feed_forward(
    x, d_inner_hid, d_hid, dropout_rate, hidden_act, param_initializer=None, name="ffn"
):
    """
...
def encoder_co_layer(
    ...
    enc_input_pre = pre_process_layer(
        enc_input, preprocess_cmd, prepostprocess_dropout, name=name + "_pre_att"
    )

    enc_input_vl_pre = pre_process_layer(
        enc_vl_input, preprocess_cmd, prepostprocess_dropout, name=name + "_vl_pre_att"
    )
    ...
    ffd_output = positionwise_feed_forward(
        pre_process_layer(
            attn_output, preprocess_cmd, prepostprocess_dropout, name=name + "_pre_ffn"
        ),
        d_inner_hid,
        d_model,
        relu_dropout,
