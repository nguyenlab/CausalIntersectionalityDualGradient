...
from src.vilio.modeling_bertV_gradient import BertV
...
class ModelV(nn.Module):
    ...
    def __init__(
        ...
        if tr_name.startswith("roberta"):
            self.model, loading_info = RobertaV.from_pretrained(
                tr_name,
            ...
            self.model, loading_info = BertV.from_pretrained(
                tr_name,
                output_loading_info=True,
                visual_embedding_dim=2048,
                output_hidden_states=True,
            ...
            self.model, loading_info = AlbertV.from_pretrained(
                tr_name,
                output_loading_info=True,
                visual_embedding_dim=2048,
                output_hidden_states=True,
    ...
    def forward(
        self, sents, visual_feats, visual_attention_mask=None, attention_weights=None
    ):
        ...
        if self.tr_name.startswith("roberta"):
            ...
            train_features, tokens = preprocess_bert(
        ...
        seq_out, _, intermediates = self.model(
            input_ids,
            attention_mask=input_mask,
            token_type_ids=segment_ids,
            visual_embeddings=img_feat,
            position_embeddings_visual=img_pos_feat,
        ...
        return output, intermediates, tokens

    def save(self, path):
        torch.save(self.model.state_dict(), os.path.join("%s_V.pth" % path))
