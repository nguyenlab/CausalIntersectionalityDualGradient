...
class HMTorchDataset(Dataset):
    def __init__(self, splits, gen_neg=True):
        super().__init__()
        self.name = splits
        self.splits = splits.split(",")
        ...
        for split in self.splits:
            if gen_neg:
                path = os.path.join("data/", f"{split}_negatives.jsonl")
            else:
        ...
        self.db = FeaturesDatabase(path=path2, annotation_db=None, feature_path=path)
    ...
    def process_img(self, iid):
        ...
        return {
            ...
            "pred_objs": torch.max(
                torch.FloatTensor(item["image_info_0"]["cls_prob"]), -1
            ).indices,
            "pred_conf": torch.max(
                torch.FloatTensor(item["image_info_0"]["cls_prob"]), -1
            ).values,
...
class HMEvaluator:
    ...
    def dump_csv(
        ...
        d = {
            "id": [str(int(tensor)).zfill(5) for tensor in id2ans.keys()],
            "proba": list(id2prob.values()),
            "label": list(id2ans.values()),
        }
        d.update(modalities)
        results = pd.DataFrame(data=d)
        results["conf_modality"] = conf_modality
