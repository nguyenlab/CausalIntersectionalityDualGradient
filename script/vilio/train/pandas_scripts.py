...
def crude_hash(img_path):
    ...
    col_val = -1
    try:
        while (not (255 > img[row_val, col_val, 0] > 1)) or (
            not (
                img[row_val, col_val, 0]
                != img[row_val, col_val, 1]
                != img[row_val, col_val, 2]
            )
        ):
            row_val += 1
            col_val -= 1
        ...
        try:
            ...
            hash_v = (
                str(img.shape[0]) + str(img.shape[1]) + str(img[row_val, col_val]) * 3
            )
            return hash_v
    ...
    hash_v = (
        str(img.shape[0])
        + str(img.shape[1])
        + str(img[row_val, col_val, 0])
        + str(img[row_val, col_val, 1])
        + str(img[row_val, col_val, 2])
    )
    return hash_v
...
def clean_data(data_path="./data"):
    ...
    ## Load all files
    train = pd.read_json(
        os.path.join(data_path, "train.jsonl"), lines=True, orient="records"
    )
    dev_seen = pd.read_json(
        os.path.join(data_path, "dev_seen.jsonl"), lines=True, orient="records"
    )
    ...
    dev_unseen = pd.read_json(
        os.path.join(data_path, "dev_unseen.jsonl"), lines=True, orient="records"
    )
    ...
    # Hash images
    train_dist["full_path"] = train_dist["img"].apply(
        lambda x: os.path.join(data_path, str(x))
    )
    ...
    pretrain.to_json(
        path_or_buf=os.path.join(data_path, "pretrain.jsonl"),
        orient="records",
        lines=True,
    )
    ...
    trainclean.to_json(
        path_or_buf=os.path.join(data_path, "train.jsonl"), orient="records", lines=True
    )

    # c) Cleaned Train + unused data from dev_unseen + dev_seen
    traincleandev = pd.concat([train, dev_unseen, dev_seen])
    traincleandev.to_json(
        path_or_buf=os.path.join(data_path, "traindev.jsonl"),
        orient="records",
        lines=True,
    )
