...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=False,
        const=True,
        help="Whether to use tsv extraction (else lmdb)",
    )
    parser.add_argument(
        "--num_features",
        type=int,
        default=100,
        help="How many features we have per img (e.g. 100, 80)",
    )
    parser.add_argument(
        "--num_pos", type=int, default=4, help="How many position feats - 4 or 6"
    )
    ...
    parser.add_argument(
        ...
        default=False,
        const=True,
        help="Applies Multi-sample dropout & Layeravg",
    )
    parser.add_argument(
        "--acc",
        type=int,
        default=1,
        help="Amount of accumulation steps for bigger batch size - make sure to adjust LR",
    )
    ...
    parser.add_argument(
        ...
        default=False,
        const=True,
        help="Combine subtrained data; subtrain must be true",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="./data",
        help="Where ckpts, csvs shall be saved to",
    )
    parser.add_argument("--tqdm", action="store_const", default=False, const=True)
    parser.add_argument(
        "--topk", type=int, default=-1, help="For testing only load topk feats from tsv"
    )
    parser.add_argument(
        "--exp", type=str, default="experiment", help="Name of experiment for csv's"
    )

    # Model Loading & Saving - Note: PATHS must be put in here!
    parser.add_argument(
        "--model", type=str, default="X", help="Type of Model, X V O U D"
    )
    parser.add_argument(
        "--tr",
        type=str,
        default="bert-base-uncased",
        help="Name of NLP transformer to be used",
    )
    parser.add_argument(
        "--midsave", type=int, default=-1, help="Save a MID model after x steps"
    )
    parser.add_argument(
        "--loadfin",
        type=str,
    ...
    parser.add_argument(
        "--loadpre", type=str, default=None, help="Load the pre-trained model."
    )
    parser.add_argument(
        "--fromScratch",
        dest="from_scratch",
    ...
    # Pre-training Config
    parser.add_argument(
        "--taskHM", dest="task_hm", action="store_const", default=False, const=True
    )
    parser.add_argument(
        "--taskMatched",
        dest="task_matched",
    ...
    parser.add_argument(
        ...
        type=str,
        default="attattr",
        choices=["attention", "gradient", "attattr"],
    )
    parser.add_argument(
        "--label_path",
        type=str,
        default="data/confounders.parquet",
    )
    parser.add_argument(
        "--summary",
        type=str,
    ...
    parser.add_argument("--preds_threshold", default=0.0, type=float)
    parser.add_argument("--logsoftmax", action="store_true")
    parser.add_argument("--pick_correct_labels", action="store_true")
    parser.add_argument("--text_only", action="store_true")
    parser.add_argument(
        "--max_highlight_text_num",
        type=int,
        default=10,
    )
    parser.add_argument(
        "--max_highlight_image_num",
        type=int,
        default=3,
    )
    parser.add_argument("--highlight_sample", action="store_true")
