...
# switched to gradient model
from src.vilio.modeling_bertO_gradient import BertO
...
class ModelO(nn.Module):
    ...
    def __init__(
        ...
        if tr_name.startswith("bert"):
            self.model, loading_info = BertO.from_pretrained(
                tr_name, output_loading_info=True, img_feature_dim=2048 + args.num_pos
            )
    ...
    def forward(
        ...
        if self.tr_name.startswith("bert") or self.tr_name.startswith("albert"):
            train_features, tokens_all = preprocess_bert(
                sents, self.max_seq_len, self.tokenizer
            )
        ...
        image_mask = torch.ones(
            (input_ids.shape[0], self.max_img_seq_len), dtype=torch.long
        )
        input_mask = torch.cat((input_mask, image_mask), dim=-1).cuda()

        seq_out, pool_out, intermediates = self.model(
            input_ids,
            token_type_ids=segment_ids,
            attention_mask=input_mask,
            img_feats=img_feat,
            # use attention weights if given
            attention_weights=attention_weights,
            is_logsoftmax=is_logsoftmax,
        )
        ...
        if args.reg:
            ...
            # hidden layer to classifier
            output = self.classifier(pool_out)

        # add encoder outputs for attention / gradient
        output = (output, intermediates, tokens_all)

        return output

    def save(self, path):
        torch.save(self.model.state_dict(), os.path.join("%s_O.pth" % path))
